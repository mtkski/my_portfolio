...
var smokeTimeout=null;
function start_smoke(duration){
    set_scroll();
    if (ie_version) setTimeout(function(){ mousedown=true; }, 51);
    else mousedown=true;
    if (smokeTimeout) clearTimeout(smokeTimeout);
    duration = (typeof duration==='number')? duration : 5000; // default 8s
    smokeTimeout = setTimeout(stop_smoke, duration);
}

function stop_smoke(){
    alert("Don't worry, I fixed it !");
    mousedown=false;
    if (smokeTimeout){ clearTimeout(smokeTimeout); smokeTimeout=null; }
    for (var i=0;i<smokeyness;i++){
        if (tokey[i]!==false){
            toke[i].visibility='hidden';
            tokey[i]=false;
        }
